...
public class Board implements Logic, java.io.Serializable {
	...
	public Board(JsonNode board) {
		size = board.get("size") == null ? null : board.get("size").asInt();
		empty = board.get("empty") == null ? null : board.get("empty").asInt();
		friends = board.get("friends") == null ? null : board.get("friends").asInt();
		if (board.get("allBlack") == null)
			allBlackLists = null;
		else {
			allBlackLists = new ArrayList<>();
			JsonNode boardI = board.get("allBlack");
			Iterator<JsonNode> iteratorI = boardI.iterator();
			while (iteratorI.hasNext()) {
				ArrayList<Point> row = new ArrayList<>();
				JsonNode boardJ = iteratorI.next();
				Iterator<JsonNode> iteratorJ = boardJ.iterator();
				while (iteratorJ.hasNext()) {
					JsonNode field = iteratorJ.next();
					row.add(new Point(field));
				}
				allBlackLists.add(row);
			}
		}
		if (board.get("allWhite") == null) {
			allWhiteLists = null;
		} else {
			allWhiteLists = new ArrayList<>();
			JsonNode myboardI = board.get("allWhite");
			Iterator<JsonNode> myiteratorI = myboardI.iterator();
			while (myiteratorI.hasNext()) {
				ArrayList<Point> row = new ArrayList<>();
				JsonNode myboardJ = myiteratorI.next();
				Iterator<JsonNode> myiteratorJ = myboardJ.iterator();
				while (myiteratorJ.hasNext()) {
					JsonNode field = myiteratorJ.next();
					row.add(new Point(field));
				}
				allWhiteLists.add(row);
			}
		}
		if (board.get("board") == null) {
			this.board = null;
		} else {
			ArrayList<ArrayList<Status>> tempBoard = new ArrayList<>();
			JsonNode boardS = board.get("board");
			Iterator<JsonNode> iteratorS = boardS.iterator();
			while (iteratorS.hasNext()) {
				ArrayList<Status> row = new ArrayList<>();
				JsonNode boardJ = iteratorS.next();
				Iterator<JsonNode> iteratorJ = boardJ.iterator();
				while (iteratorJ.hasNext()) {
					JsonNode field = iteratorJ.next();
					row.add(Status.fromInt(field.asInt()));
				}
				tempBoard.add(row);
			}
			...
			for (int i = 0; i < tempBoard.size(); i++)
				for (int j = 0; j < tempBoard.size(); j++) {
					this.board[i][j] = tempBoard.get(i).get(j);
				}
		}
		if (board.get("friends") == null) {
			friendsLocation = null;
		} else {
			ArrayList<Point> mytempBoard = new ArrayList<>();
			JsonNode boardF = board.get("friends");
			Iterator<JsonNode> iteratorF = boardF.iterator();
			while (iteratorF.hasNext()) {
				JsonNode field = iteratorF.next();
				Point row = new Point(field);
				mytempBoard.add(row);
			}

			friendsLocation = new Point[size];
			for (int i = 0; i < mytempBoard.size(); i++)
				friendsLocation[i] = mytempBoard.get(i);
		}
	...
	public JsonNode toJson() {
		...
		jsonNode.put("empty", empty);
		jsonNode.put("friends", friends);
		ArrayNode boardI = mapper.createArrayNode();
		for (int i = 0; i < size; i++) {
			ArrayNode boardJ = mapper.createArrayNode();
			for (int j = 0; j < size; j++) {
				boardJ.add(board[i][j].toInt());
			}
			boardI.add(boardJ);
		}
		...
		for (int i = 0; i < allBlackLists.size(); i++) {
			...
			for (int j = 0; j < allBlackLists.get(i).size(); j++) {
				allBlackJ.add(allBlackLists.get(i).get(j).toJson());
			}
			allBlackI.add(allBlackJ);
		}
		...
		for (int i = 0; i < allWhiteLists.size(); i++) {
			...
			for (int j = 0; j < allWhiteLists.get(i).size(); j++) {
				allWhiteJ.add(allWhiteLists.get(i).get(j).toJson());
			}
			allWhiteI.add(allWhiteJ);
		}
	...
	/**
	 * create new chain of size 1 when pawn hasn't friends around or connect
	 * chains when pawn has friends
	 */
	@Override
	...
	/**
	 * create new chain (insideList) and add it to certain outsideList depending
	 * on the pawn's color
	 */
	@Override
	...
	/**
	 * connect one chain with two or more having the same pawn's color, create
	 * new list (masterList) to merge all points and remove old list
	 */
	@Override
	...
	/**
	 * at position y, x for certain player check if there are friendly pawns. If
	 * so, find their chains (lists), count breath for the whole chain. If
	 * parameter remove is true, each chain with 0 breaths is deleted.
	 * 
	...
	public boolean checkBreath(int y, int x, Status color, boolean remove) {
		...
		if (friends > 0) {
			...
			for (int j = 0; j < numberOfFriends; j++) { // for all friends
				for (List<Point> aFriendsList : friendsList) { // for each
																// element
																// external list
					for (int i = 0; i < aFriendsList.size(); i++) {// for each
																	// element
																	// of list
																	// of
																	// internal
						if (aFriendsList.get(i).getX() == friendsLocation[j].getX()
								&& aFriendsList.get(i).getY() == friendsLocation[j].getY()) {// coordinate
																								// X
																								// and
																								// Y
																								// element
																								...
																								// friend
																								// near
							toRemoveMap.put(j, aFriendsList); // remember
																// list(chain)
																// friend at
																// index 'j'
							for (int k = 0; k < aFriendsList.size(); k++) {
								...
								lookAround((int) aFriendsList.get(k).getX(), (int) aFriendsList.get(k).getY(), color,
										true); // empty fields are checked and
												// selected how status.checked
								sumEmpty[j] = sumEmpty[j] + empty; // couting
																	// free
																	// field
																	// around
																	// every
				...
				if (remove) {
					if (sumEmpty[j] == 0) { // sum empty filed around chain at
											// index 'j'=0
	...
	public void removeList(int index, Map<Integer, List<Point>> map, List<List<Point>> list) {
		for (Point point : map.get(index)) { // for every element memorized list
												// at list 'j'
			board[(int) point.getX()][(int) point.getY()] = Status.empty; // after
																			// the
																			// suffocation
																			// chain
		}
	...
	/**
	 * check if the move is allowed, because of KO, or not allowed meaning
	 * suicide.
	 * 
	 * @return allow
	 */
	...
	/**
	 * at position y, x, scan surrounding fields and count empty places, number
	 * of pawn's with the same colors (friends) and get these friends location.
	 * 
	 * @param markField
	 *            used to prevent scanning the same field for empty place more
	 *            than once during checkBreath procedure. it changes status of
	 *            field from empty to checked.
	 */
	@Override
